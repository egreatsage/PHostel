...
import { MdAirplaneTicket, MdOutlineDeleteForever } from "react-icons/md";
...
const Rooms = ({ id,setRoomId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setmessage("");
    const newRoom = {
    roomno,gender,category,pricing,
    };
    console.log(newRoom)
    try {
      if (id !== undefined && id !== "") {
        await dbdataservice.updateRoom(id, newRoom);
        // setmessage({ error: false, msg: "room updated!" });
        Swal.fire({
          icon:'success',
          title: 'Success',
    ...
    setRoomno(""); setGender("");setCategory("");setPricing("");
  };

  const editHandler = async () => {
    setmessage("");
    try {
      const docSnap = await dbdataservice.getRoom(id);
   ...
   <div className="mt-16 flex justify-end w-12">
    ...
    <header>
          ...
          <div className='md:mx-10 mx-2 mt-20 my-10'>
            <h1 className="text-start font-semibold tracking-wider text-xl">Add a Room</h1>
      <form onSubmit={handleSubmit} >
        <div className='grid md:py-6 w-full sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 overflow-x-hidden'>
        <div className='my-3'><Input
         value={roomno}
         onChange={(e)=>setRoomno(e.target.value)}
        color='teal' type='text' className='text-black' variant='standard' label='Room Number' required/></div>
         <div className='my-3'><Input color='teal' type='tel' className='text-black' variant='standard' label='Gender'
       value={gender}
       onChange={(e)=>setGender(e.target.value)} required/></div>
      <div className='my-3'><Input color='teal' type='text' className='text-black' variant='standard' label='Category'
       value={category}
       onChange={(e)=>setCategory(e.target.value)}
       required/></div>
      <div className='my-3'><Input color='teal' type='number' className='text-black' variant='standard' label='Pricing'
       value={pricing}
      ...
      <h1 className="text-start font-semibold tracking-wider text-xl">Room Information</h1>
      <div className='overflow-hidden'>
         <div className="flex justify-between my-3">
        <div className='md:ml-8'>
        <Link to='/addbooking' className='text-md mx-2 hover:underline hover:font-bold'>Add</Link>
          <button className='text-md hover:font-bold hover:underline'  onClick={getAllRooms} > Refresh</button>
